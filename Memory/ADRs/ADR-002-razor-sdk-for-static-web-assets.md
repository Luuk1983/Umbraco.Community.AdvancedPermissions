...
The main package project (`UmbracoAdvancedSecurity`) needs to expose frontend files in `wwwroot/App_Plugins/` so they are served correctly when the package is consumed by a host application. Initially the project used `Microsoft.NET.Sdk`.
...
Change the SDK in `UmbracoAdvancedSecurity.csproj` from `Microsoft.NET.Sdk` to `Microsoft.NET.Sdk.Razor`:
