        ...
        CancellationToken cancellationToken)
    {
        foreach (var group in notification.SavedEntities)
        {
            try
            {
                ...
                // permissions also include verbs outside our model (e.g. Umb.Document.PropertyValue.*),
                // which we neither resolve nor display. Storing one would later be re-sent by the editor
                // and rejected by the save endpoint ("Invalid verb").
