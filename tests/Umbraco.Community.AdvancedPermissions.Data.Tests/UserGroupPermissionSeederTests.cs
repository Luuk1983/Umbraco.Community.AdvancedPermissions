...
public sealed class UserGroupPermissionSeederTests : IAsyncLifetime
{
    ...
    /// (Umbraco's property-value verbs) must seed ONLY the verbs in
    /// <see cref="AdvancedPermissionsConstants.AllVerbs"/>. Storing an unmanaged verb would later be re-sent by
    /// the editor and rejected by the save endpoint's verb validation (HTTP 400 "Invalid verb").
    /// </summary>
    [Fact]
    public async Task Handle_NewGroupWithUnmanagedVerbs_SeedsOnlyManagedVerbs()
    {
        ...
        Assert.DoesNotContain("Umb.Document.PropertyValue.Write", verbs);
    }
    ...
    {
        var group = BuildGroup("editors", ["Umb.Document.PropertyValue.Read", "Umb.Document.PropertyValue.Write"]);
