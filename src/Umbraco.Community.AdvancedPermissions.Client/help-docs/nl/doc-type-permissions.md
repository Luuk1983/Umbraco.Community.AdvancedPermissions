...
Aanmaken is standaard toegestaan (overal waar Umbraco's eigen toegestane onderliggende typen dat toelaten). Deze editor is een **filter, geen toekenning** en werkt daarmee anders dan de boomgebaseerde rechten: hij beperkt alleen de documenttypen die Umbraco al aanbiedt. Je voegt een **Weigeren** toe om een type voor een groep weg te nemen, of een **Toestaan** om het te behouden — maar een Toestaan laat nooit een type verschijnen dat Umbraco daar niet al als onderliggend type toestaat.
...
- Omdat aanmaken standaard is toegestaan, voeg je meestal **Weigeren** toe om specifieke typen te vergrendelen. Een **Toestaan** behoudt alleen een type dat Umbraco daar al toestaat (of wint het terug met Prioriteitsoverschrijving) — het kan geen type toevoegen dat sowieso geen geldig onderliggend type is.
