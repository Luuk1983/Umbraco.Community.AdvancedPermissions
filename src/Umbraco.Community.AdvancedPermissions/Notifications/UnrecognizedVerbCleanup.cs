...
/// save endpoint reject the whole save with "Invalid verb". Such rows can leak when Umbraco adds default
/// permission verbs that an earlier package version copied verbatim (e.g. <c>Umb.Document.PropertyValue.*</c>).
/// This handler heals those installs automatically. It is idempotent and cheap, so it runs on every boot,
/// guarded like the data import so only the writable main instance acts.
/// </remarks>
