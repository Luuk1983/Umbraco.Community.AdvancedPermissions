...
[ApiVersion("1.0")]
public sealed class AdvancedSecurityMetaController(IUserGroupService userGroupService)
    : AdvancedSecurityControllerBase
{
    ...
    public IActionResult GetVerbs() =>
        Ok(AdvancedSecurityConstants.AllVerbs.Select(v => new VerbResponseModel(v, GetVerbDisplayName(v))).ToList());
