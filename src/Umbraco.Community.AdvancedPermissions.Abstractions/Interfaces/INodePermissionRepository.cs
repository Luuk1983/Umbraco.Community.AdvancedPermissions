...
/// <summary>
/// Provides data access for node-keyed advanced security permission entries.
/// </summary>
/// <remarks>
/// <para>
/// This is the storage contract shared by every node-based permission target — content documents
/// (<see cref="IAdvancedPermissionRepository"/>) and library elements/folders
/// (<see cref="IElementPermissionRepository"/>). The shape is entirely generic: an entry is keyed on
/// a node <see cref="Guid"/>, a role alias, and a verb, with a state and scope. The concrete
/// implementation binds the contract to a specific backing table.
/// </para>
/// <para>
/// "Node" here is deliberately abstract — it is a content node for the content target and a library
/// element or element-container for the element target. Use
...
public interface INodePermissionRepository
{
    ...
    /// Used when a node is permanently deleted.
    /// </summary>
    /// <param name="nodeKey">The node key to remove all entries for.</param>
    /// <param name="cancellationToken">Token to support cancellation.</param>
