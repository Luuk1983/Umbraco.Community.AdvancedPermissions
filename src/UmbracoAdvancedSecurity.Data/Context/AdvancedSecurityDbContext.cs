...
/// Manages the <c>AdvancedSecurityPermission</c> table that stores all explicit permission entries.
/// </summary>
/// <remarks>
/// Provider-specific derived contexts (<see cref="AdvancedSecurityDbContextSqlite"/> and
/// <see cref="AdvancedSecurityDbContextSqlServer"/>) handle database-provider configuration.
/// At runtime, Umbraco's connection string is provided via <c>UseUmbracoDatabaseProvider</c>.
/// </remarks>
public class AdvancedSecurityDbContext : DbContext
{
    /// <summary>
    ...
    public AdvancedSecurityDbContext(DbContextOptions<AdvancedSecurityDbContext> options)
        : base(options)
    {
    }
    ...
    protected AdvancedSecurityDbContext(DbContextOptions options)
        : base(options)
    {
    }
    ...
    private static void ConfigurePermissions(ModelBuilder modelBuilder)
    {
        ...
        {
            entity.ToTable("AdvancedSecurityPermission");
