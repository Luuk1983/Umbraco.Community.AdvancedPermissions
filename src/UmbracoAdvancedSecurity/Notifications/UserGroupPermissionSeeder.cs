...
public sealed class UserGroupPermissionSeeder(
    IDbContextFactory<AdvancedSecurityDbContext> dbContextFactory,
        ...
        CancellationToken cancellationToken)
    {
        foreach (var group in notification.SavedEntities)
        {
            try
            {
                ...
                {
                    logger.LogDebug(
                        "Advanced Security: New group '{Group}' has no native permissions — starting with Deny all",
                        group.Alias);
                    continue;
                }
                ...
                logger.LogInformation(
                    "Advanced Security: Seeded {Count} root permission entries for new group '{Group}'",
                    entriesToAdd.Count,
                    group.Alias);
            }
            catch (Exception ex)
            {
                logger.LogWarning(
                    ex,
                    "Advanced Security: Failed to seed permissions for group '{Group}'. " +
