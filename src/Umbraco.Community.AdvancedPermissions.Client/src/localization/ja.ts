...
export default {
  uap: {
    ...
    help_libraryPermissions_description: 'ライブラリツリー全体で、ユーザーグループごとに許可／拒否のアクセス権限を管理します。',
    help_elementTypePermissions_description: '各ユーザーグループがライブラリで使用できるエレメントタイプを制御します。',
    help_libraryAccessViewer_description: '各ノードでユーザーまたはユーザーグループが持つ有効なライブラリのアクセス権限を確認します。',
    help_libraryInsertViewer_description: 'ライブラリでユーザーまたはユーザーグループが挿入できるエレメントタイプを監査します。',
    ...
    previewElementFolderSet: (action: string) => `このフォルダーには適用されません。その中のエレメントで${action}します。`,
    previewElementFolderInherit: 'このフォルダーには適用されません。その中のエレメントは親ノードから継承します。',
    dialogElementFolderNote: (verb: string) => `「${verb}」はフォルダーには適用されません — その中のエレメントにのみ適用されます。`,
    ...
    library_accessViewerHeadline: 'ライブラリアクセスビューア',
    library_nodeHeader: 'ライブラリアイテム',
    library_root: 'デフォルトのアクセス権限',
    library_selectRolePrompt: 'ライブラリのアクセス権限を管理するには、ユーザーグループを選択します。',
    library_permissionsSaved: 'ライブラリのアクセス権限を保存しました。',
    ...
    elementTypePermissions_noTypes: 'ライブラリのエレメントタイプが見つかりません。ドキュメントタイプをエレメントタイプとしてマークし、「ライブラリで許可」を有効にしてください。',
    elementType_dialogHeadline: (type: string) => `ライブラリでの「${type}」の作成アクセス権限`,
    elementType_inheritLabel: '未設定（デフォルトで許可）',
    elementType_allowLabel: '許可（ライブラリで作成可能）',
